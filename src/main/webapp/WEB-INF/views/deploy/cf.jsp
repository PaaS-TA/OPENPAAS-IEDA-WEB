...
//MODIFY
function getCfAwsData(record){
	var url = "/cf/aws/"+record.id;
	$.ajax({
		type : "GET",
		url : url,
...
function getCfOpenstackData(record){
	var url = "/cf/openstack/"+record.id;
	$.ajax({
		type : "GET",
		url : url,
		contentType : "application/json",
...
function getKeyPathFileList(){
	$.ajax({
		...
		error : function( e, status ) {
			w2alert("Private Key 목록조회 실패하였습니다.", "CF 설치");
		}
	});
}
...
function keyPathFileUpload(){
	var form = (iaas == "AWS") ? $(".w2ui-msg-body #awsForm")[0] : $(".w2ui-msg-body #osCfForm")[0];
	...
	$.ajax({
		...
		error : function( e, status ) {			
			w2alert(iaas + " 설정 등록에 실패 하였습니다.", "CF 설치");
		}
	});
}
...
function settingAWSData(contents){
	...
	resourceInfo = {
			...
			cfPassword		: contents.cfPassword
	}
	...
	$("#cfInfoDiv").w2popup({
		...
		onOpen:function(event){
			event.onComplete = function(){
				...
				if(cfInfo != ""){
					$(".w2ui-msg-body input[name='deploymentName']").val(cfInfo.deploymentName);
					$(".w2ui-msg-body input[name='directorUuid']").val($("#directorUuid").text());
					$(".w2ui-msg-body input[name='releaseVersion']").data('selected', {text:cfInfo.releaseVersion});
				} else {
					$(".w2ui-msg-body input[name='directorUuid']").val($("#directorUuid").text());
				}
			}
...
function saveCfInfo(type){
	...
	if ( type == 'after' ) {
		...
		if( checkEmpty(cfInfo.releaseVersion) ){
			w2alert("배포에 사용할 CF 릴리즈를 선택하세요.", "" , function(){
		...
		$.ajax({
			...
			error : function( e, status ) {
				w2alert("기본정보 저장 실패하였습니다.", "Cf 설치(AWS)");
			}
		});
	}
	else if(type == 'before'){
		...
		onOpen:function(event){
			event.onComplete = function(){				
				if(networkInfo != ""){
					...
					$(".w2ui-msg-body input[name='publicStaticIp']").val(cfInfo.publicStaticIp);
		...
		onOpen:function(event){
			event.onComplete = function(){
				...
				if(resourceInfo != ""){
					...
					$(".w2ui-msg-body input[name='cfPassword']").val(resourceInfo.cfPassword);
				}
			}
		},
		onClose : initSetting
...
function getDeployInfo(){
	...
	var url = (iaas == "AWS") ? "/cf/getAwsCfDeployInfo": "/cf/getOpenstackCfDeployInfo";
	console.log(" URL ::: " + url );
	$.ajax({
		type : "POST",
		url : url,
...
function installPopup(){
	...
	var message = "CF(배포명:" + cfInfo.deploymentName +  ") ";
...
// Openstack CF INFO 
function osCfInfoPopup(){
	$("#osCfInfoDiv").w2popup({
		width 	: 670,
		height	: 450,
		title   : "CF설치 (OPENSTACK)",
		modal	: true,
		onOpen:function(event){
			event.onComplete = function(){				
				$(".w2ui-msg-body input[name='releaseVersion']").w2field('list', { items: releases , maxDropHeight:200, width:250});
...
function getReleaseVersionList(){
	$.ajax({
		...
		error : function( e, status ) {
			w2alert("Release Version List 를 가져오는데 실패하였습니다.", "CF 설치");
		}
	});
}
...
<div id="main">
	<div class="page_site">설치관리자 환경설정 > <strong>Cf 설치</strong></div>
	...
	<div id="cfInfoDiv" style="width:100%;height:100%;" hidden="true">
		...
		<div rel="body" style="width:100%;height:100%;padding:15px 5px 0 5px;margin:0 auto;">
		    ...
		    <div class="w2ui-page page-0" style="padding-left:5%;">
		        ...
		        <div class="w2ui-field">
		            <label style="text-align: left;width:250px;font-size:11px;">CF 릴리즈</label>
		            <div>
		...
		<div rel="body" style="width:100%;height:100%;padding:15px 5px 0 5px;margin:0 auto;">
			...
			<div class="w2ui-page page-0" style="padding-left: 5%;">
				...
				<div class="w2ui-field">
					...
					<div>
						<input name="cfPassword" type="text" style="float:left;width:330px;" placeholder="VM인스턴스의 비밀번호를 입력하세요."/>
					</div>
				</div>
	...
	<div id="osCfInfoDiv" style="width:100%;height:100%;" hidden="true">
		...
		<div rel="body" style="width:100%;height:100%;padding:15px 5px 0 5px;margin:0 auto;">
			...
			<br/>
		    <div class="w2ui-buttons" rel="buttons" hidden="true">
		    	<button class="btn" style="float: left;" onclick="saveOsCfInfo('before');">이전</button>
		        <button class="btn"  onclick="popupComplete();">취소</button>
		        <button class="btn" style="float: right;padding-right:15%" onclick="saveOsCfInfo('after');">다음>></button>
		    </div>
		</div>
	</div>	
