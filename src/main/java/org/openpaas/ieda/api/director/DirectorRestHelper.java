...
public class DirectorRestHelper {
	...
	public static String getDeleteReleaseURI(String host, int port, String releaseName, String releaseVersion) {
		return UriComponentsBuilder.newInstance().scheme("https").host(host).port(port).path("/releases/{name}")
				.queryParam("recreate", "true").build().expand(releaseName).toUri()
				.toString();
	}
	...
	public static String getDeleteDeploymentURI(String host, int port, String deploymentName) {
		return UriComponentsBuilder.newInstance().scheme("https").host(host).port(port).path("deployments/{name}")
				.queryParam("force", "true").build().expand(deploymentName).toUri()
				.toString();
	}
	...
	public static void trackToTask(IEDADirectorConfig defaultDirector, SimpMessagingTemplate messageTemplate, String messageEndpoint, HttpClient client, String taskId) {
		...
		try {
			...
			while ( true ) {
				// Task 상태 조회
				GetMethod getTaskStausMethod = new GetMethod(DirectorRestHelper.getTaskStatusURI(defaultDirector.getDirectorUrl(), defaultDirector.getDirectorPort(), taskId));
				getTaskStausMethod= (GetMethod)DirectorRestHelper.setAuthorization(defaultDirector.getUserId(), defaultDirector.getUserPassword(), (HttpMethodBase)getTaskStausMethod);
				...
				if ( HttpStatus.valueOf(statusCode) != HttpStatus.OK ) {
					...
					sendTaskOutput(messageTemplate, messageEndpoint, "error", Arrays.asList("Task " +  taskId + " : 상태 조회 중 오류가 발생하였습니다."));
					break;
				}
				...
				// Task Output 조회
				GetMethod getTaskOutputMethod = new GetMethod(DirectorRestHelper.getTaskOutputURI(defaultDirector.getDirectorUrl(), defaultDirector.getDirectorPort(), taskId, "event"));
				getTaskOutputMethod= (GetMethod)DirectorRestHelper.setAuthorization(defaultDirector.getUserId(), defaultDirector.getUserPassword(), (HttpMethodBase)getTaskOutputMethod);
					...
					for ( TaskOutput output : taskOutputList ) {
						...
						if ( output.getStage() != null ) {
							...
							if ( error != null ) {
								...
								System.out.println("  Error Code : " + error.get("error") + ", Message :" + error.get("message"));
							}
						}
			...
			e.printStackTrace();
			sendTaskOutput(messageTemplate, messageEndpoint, "error", Arrays.asList("", "An exception occurred while executing the task " + taskId) );
		}
